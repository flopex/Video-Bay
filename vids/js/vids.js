var videoViewer = {
	...
	showPlayer : function() {
		...
		var html5LOC = OC.filePath('vids', 'js', 'jwplayer.html5.js');
		...
		jwplayer("videoStream").setup({
			...
			html5player: html5LOC,
			flashplayer: flashLOC,
			file: streamPath,
			width: 640,
			height: 480,
			autostart: true
...
$(document).ready(function() {
	...
	var webRoot = '/'+OC.filePath('vids','stos','.').split('/')[1];
	...
	$.getJSON(OC.filePath('vids', 'ajax', 'getvideos.php')).then(function (data) {
		...
		$.each(data.videos,function(k,v){
			...
			var link = $('<a href="#"></a>');
			link.attr('name',webRoot+'/index.php/apps/files/download/'+downPath);
